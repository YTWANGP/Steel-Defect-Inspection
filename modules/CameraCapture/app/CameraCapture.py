...
class CameraCapture(object):
    ...
    def set_do(self, parser_result):
        #print("Displaying " + parser_result)
        url = "http://192.168.4.100/api/slot/0/io/do"
        if "normal" in parser_result.lower():
            payload = "{\"slot\":0,\"io\":{\"do\":[{\"doIndex\":0,\"doMode\":0,\"doStatus\":0},{\"doIndex\":1,\"doMode\":0,\"doStatus\":0},{\"doIndex\":2,\"doMode\":0,\"doStatus\":1},{\"doIndex\":3,\"doMode\":0,\"doStatus\":0}]}}"
        else:
            payload = "{\"slot\":0,\"io\":{\"do\":[{\"doIndex\":0,\"doMode\":0,\"doStatus\":1},{\"doIndex\":1,\"doMode\":0,\"doStatus\":0},{\"doIndex\":2,\"doMode\":0,\"doStatus\":0},{\"doIndex\":3,\"doMode\":0,\"doStatus\":0}]}}"
        headers = {
            'Accept': "vdn.dac.v1",
    ...
    def processing_do(self, frameCounter):
        url = "http://192.168.4.100/api/slot/0/io/do"
        if self.isWebcam or frameCounter % 15 == 0:
            payload = self.get_do()
            #print(payload)
            if payload.find('{\"doIndex\":1,\"doMode\":0,\"doStatus\":0}'):
                payload = payload.replace('{\"doIndex\":1,\"doMode\":0,\"doStatus\":0}','{\"doIndex\":1,\"doMode\":0,\"doStatus\":1}')
            else:
                payload = payload.replace('{\"doIndex\":1,\"doMode\":0,\"doStatus\":1}','{\"doIndex\":1,\"doMode\":0,\"doStatus\":0}')
            headers = {
                'Accept': "vdn.dac.v1",
                'Content-Type': "application/json",
                'Content-Length': str(len(payload))
    ...
    def start(self):
        ...
        while True:
            ...
            frameCounter += 1
            #Display processing LED
            self.processing_do(frameCounter)
            if self.isWebcam:
                frame = self.vs.read()
            else:
            ...
            #Process externally
            if self.isWebcam or frameCounter % 30 == 1:
                if self.imageProcessingEndpoint != "":
                    ...
                    parser_result = self.MessageParser.highestProbabilityTagMeeting(json.loads(response))
                    
                    #Display LED
                    self.set_do(parser_result)
                    
                    timestr = datetime.now().strftime("%Y%m%d%H%M%S%f")
                    currtime = time.strftime("%Y-%m-%d %H:%M:%S")
                    inspection_result = '[{"msg_type": "steel_inspection", "time": "' + currtime + '", "frame": "' + timestr + '", "inspection_result": "'+ parser_result + '", "RootCauseId": '
                    if parser_result == 'normal':
                        inspection_result += '0'
                    else: 
                        inspection_result += str(randint(1, 4))
                    inspection_result += '}]'
                    ...
                    if response != "[]" and self.sendToHubCallback is not None:
                        ...
                        if parser_result == "defect":
                            local_path = '/media/' + timestr + ".jpg"
                            cv2.imwrite(local_path, frame)
                        print("Msg to IoTHub " + inspection_result)
                        self.sendToHubCallback(inspection_result)
